...
private:
    ...
    int helper(TreeNode* root, const int& total, long& res) {
        if (!root) return 0;
        int left = helper(root->left, total, res);
        int right = helper(root->right, total, res);
        res = max(res, max((long)(total-left)*left, (long)(total-right)*right));
